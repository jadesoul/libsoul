 ...
 * 
 * Date: 2012-12-25 20:25:22.944412
...
int example1(int argc, const char* argv[]) {
	...
	if (optargs.get("verbose")) {
		std::cout << optargs["filename"] << std::endl;
		for (uint i=0; i<optargs.arg_size(); ++i) std::cout<<"arg"<<i+1<<"="<<optargs[i]<<std::endl;
		return 0;
	}
	return 0;
}
...
int example2(int argc, const char *argv[]) {
	...
	optparse::OptionArgs options = parser.parse_args(argc, argv);
...
int example3(int argc, const char *argv[]) {
	...
	// test all actions
	parser.add_option("-a", "--clear") .action("store_false") .dest("no_clear") .help("clear (default)");
	parser.add_option("-b", "--no-clear") .action("store_true") .help("not clear");
	...
	OptionArgs& options = parser.parse_args(argc, argv);
	vector<string> args = parser.args();

	//cout<<"jadesoul: args.size()="<<args.size()<<endl;
	//if (args.size()==0) {
	//	parser.print_help();
	//	return 1;
	//}
	...
	{
		Output out(", ");
		for (OptionArgs::iterator it = options.all("more_milk").begin(); it != options.all("more_milk").end(); ++it)
			out(*it);
	}
...
int main(int argc, const char *argv[]) {
	// return example1(argc, argv);
	// return example2(argc, argv);
	return example3(argc, argv);
}

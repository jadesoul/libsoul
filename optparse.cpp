 ...
 * 
 * Date: 2012-12-25 20:29:08.360042
...
/// the implementation starts from here
/// by jadesoul
namespace optparse {
...
// #endif
	...
	OptionParser& OptionParser::add_option_group(const OptionGroup& group) {
		for (std::list<Option>::const_iterator oit = group._opts.begin(); oit != group._opts.end(); ++oit) {
			...
			for (std::set<string>::const_iterator it = option._short_opts.begin(); it != option._short_opts.end(); ++it)
				_optmap_s[*it] = &option;
			for (std::set<string>::const_iterator it = option._long_opts.begin(); it != option._long_opts.end(); ++it)
				_optmap_l[*it] = &option;
		}
	...
	OptionArgs& OptionParser::parse_args(const int argc, char const* const* const argv) {
		if (prog() == "")
			prog(basename(argv[0]));
		return parse_args(&argv[1], &argv[argc]);
	}
	OptionArgs& OptionParser::parse_args(const vector<string>& v) {
		...
		_values.args.assign(_leftover.begin(), _leftover.end());
		return _values;
	}
	...
	////////// class OptionArgs { //////////
	const string& OptionArgs::operator[] (const string& d) const {
		strMap::const_iterator it = _map.find(d);
		static const string empty = "";
		return (it != _map.end()) ? it->second : empty;
	}
	void OptionArgs::is_set_by_user(const string& d, bool yes) {
		if (yes)
			_userSet.insert(d);
		else
			_userSet.erase(d);
